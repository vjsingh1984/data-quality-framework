...
from pyspark.sql.types import StructType, StringType, VarcharType, CharType, IntegerType, LongType, ShortType, \
    FloatType, DoubleType, BooleanType, DecimalType, ByteType, BinaryType, \
    TimestampType, DateType, TimestampNTZType
import json, re
...
class SchemavalidationCheck:
    ...
    def get_ref_cached_data(self, full_tablename, ref_column):
        ...
        """
        threshold_list_count = self._schema_config.get(constants.SCHEMA_VALIDATION_CHECK_FK_THRESHOLD_COUNT_KEY, 
            constants.SCHEMA_VALIDATION_CHECK_FK_THRESHOLD_COUNT_VALUE) 
        use_list_check = self._schema_config.get(constants.SCHEMA_VALIDATION_CHECK_FK_USE_LIST_KEY, True)
        cache_key = f"{full_tablename}.{ref_column}"
        if cache_key not in self._ref_cache: 
            # Only calculate if cache key is missing      
            ref_df = self._spark_session.sql(f"SELECT {ref_column} AS {ref_column}_alias FROM {full_tablename}")
    ...
    def get_joined_dataframe_from_foreign_key_constraints(self, df: DataFrame):
        ...
        joined_df = df
        for fk in self._schema_config.get(constants.SCHEMA_VALIDATION_FK_CONSTRAINTS, []):
            ...
            if not(cached_data[constants.SCHEMA_VALIDATION_CHECK_FK_USE_LIST_KEY]):
                ...
                joined_df = joined_df.join(ref_df, joined_df[src_column] == ref_df[f"{ref_column}_alias"], "left_outer")
    ...
    def create_foreign_key_checks(self, check: Check, df: DataFrame):
        ...
        for fk in self._schema_config.get(constants.SCHEMA_VALIDATION_FK_CONSTRAINTS, []):
            ...
            if cached_data[constants.SCHEMA_VALIDATION_CHECK_FK_USE_LIST_KEY]:
                ...
                ref_values = ref_df.select(f"{ref_column}_alias").distinct().rdd.flatMap(lambda r: [str(r[0])]).collect()
                check = check.isContainedIn(column = src_column, allowed_values = ref_values, assertion = lambda k: k==1.0, 
                        hint= f"{src_column} must exist in {full_tablename}.{ref_column}")
            else:
                check = check.satisfies(columnCondition = f"{ref_column}_alias IS NOT NULL",
    ...
    def _get_typeofcheck_data_type(self,data_type, parameters):
        lookupvalue = constants.SCHEMA_VALIDATION_TYPEOF_DATATYPE_MAP.get(data_type,"unknown")
    ...
    def _apply_spark_datatype_checks(self, checks, tableSchema):
        ...
        not_null_contraints = self._schema_config.get(constants.SCHEMA_VALIDATION_NOT_NULL_COLUMNS_KEY, [])
        for field in tableSchema.fields:
            ...
            if nullable:
                nullable = not(column_name in not_null_contraints)
            ...
            data_type, parameters = self._extract_basetype_and_length(datatype = str(field_data_type))

            override = next((o for o in self._schema_config.get(constants.SCHEMA_VALIDATION_OVERRIDE_KEY,[]) if column_name == o['column']) ,None)

            if self._single_check_mode:
                if data_type in constants.SCHEVA_VALIDATION_PYDEEQU_DATATYPE_MAP:
                    # Apply standard datatype checks directly supported within PyDeequ hasDataType constraint using  ConstrainableDataTypes 
                    if override:
                        if constants.SCHEMA_VALIDATION_OVERRIDE_CONFIG_PATTERN_KEY in override:
                            pattern_regex = override.get(constants.SCHEMA_VALIDATION_OVERRIDE_CONFIG_PATTERN_KEY)
                            checks = checks.hasPattern(column = column_name, pattern = pattern_regex, assertion = assertion_lambda, name = f"override check for column {column_name}", hint=f"{hint_expr}. Pattern={pattern_regex}")
                            if not(override.get(constants.SCHEMA_VALIDATION_OVERRIDE_CONFIG_REPLACE_KEY,True)):
                                # This means that pattern check is applied on top of data type.
                                mapped_constrainable_datatype = constants.SCHEVA_VALIDATION_PYDEEQU_DATATYPE_MAP.get(data_type)
                                if not nullable:
                                    checks = checks.hasDataType(column = column_name, datatype = mapped_constrainable_datatype, assertion = assertion_lambda, hint = hint_expr)
                                else:
                                    typeofcheck_data_type = self._get_typeofcheck_data_type(data_type,parameters)
                ...
                elif data_type in constants.SCHEVA_VALIDATION_CASTSPARKSQL_DATATYPE_MAP:
                    cast_sparksql_type = constants.SCHEVA_VALIDATION_CASTSPARKSQL_DATATYPE_MAP.get(data_type)
                    cast_datatype_expr = f"{column_name} is NULL OR CAST({column_name} AS {cast_sparksql_type}) IS NOT NULL"
                    if not nullable:
                        cast_datatype_expr = f"CAST({column_name} AS {cast_sparksql_type}) IS NOT NULL"
                    checks = checks.satisfies(columnCondition = cast_datatype_expr,
                ...
                if parameters:
                    if data_type in ["CharType", "VarcharType"]:
                       checks = checks.hasMaxLength(column = column_name, assertion = lambda l : l <= parameters[0], hint = hint_expr)
                        ...
                        checks = checks.satisfies(columnCondition = cast_decimal_expr, 
                ...
                check = Check(spark_session = self._spark_session, level = CheckLevel.Error, description= "Schema Validation")
                if data_type in constants.SCHEVA_VALIDATION_PYDEEQU_DATATYPE_MAP:
                    if override:
                        if constants.SCHEMA_VALIDATION_OVERRIDE_CONFIG_PATTERN_KEY in override:
                            pattern_regex = override.get(constants.SCHEMA_VALIDATION_OVERRIDE_CONFIG_PATTERN_KEY)
                            check = check.hasPattern(column = column_name, pattern = pattern_regex, assertion = assertion_lambda, name = f"override check for column {column_name}", hint=f"{hint_expr}. Pattern={pattern_regex}")
                            if not(override.get(constants.SCHEMA_VALIDATION_OVERRIDE_CONFIG_REPLACE_KEY,True)):
                                if not nullable:
                                    # This means that pattern check is applied on top of data type.
                                    mapped_constrainable_datatype = constants.SCHEVA_VALIDATION_PYDEEQU_DATATYPE_MAP.get(data_type)
                                    check = check.hasDataType(column = column_name, datatype = mapped_constrainable_datatype, assertion = assertion_lambda, hint = hint_expr)
                                else:
                                    typeofcheck_data_type = self._get_typeofcheck_data_type(data_type, parameters)
                    ...
                    if not nullable:
                        cast_datatype_expr = f"CAST({column_name} AS {cast_sparksql_type}) IS NOT NULL"
                    check = check.satisfies(columnCondition = cast_datatype_expr,
                        constraintName = f"column[{column_name}] constaint for datatype[{cast_sparksql_type}] AND nullable[{nullable}]",
                        assertion = assertion_lambda, 
                        hint = hint_expr)
                #if isinstance(data_type, StringType) or isinstance(data_type, CharType) or isinstance(data_type, VarcharType):
                ...
                if parameters:
                    if data_type in ["CharType", "VarcharType"]:
                        checks.append(Check(spark_session = self._spark_session, level = CheckLevel.Error, description= "Schema Validation Max Length Check").hasMaxLength(column = column_name, assertion = lambda l : l <= parameters[0], hint = hint_expr))
                    elif data_type == "DecimalType":
                        precision, scale = parameters
                        cast_decimal_expr = f"{column_name} is NULL OR CAST({column_name} AS DECIMAL({precision},{scale})) IS NOT NULL"
                        if not nullable:
                            cast_decimal_expr = f"CAST({column_name} AS DECIMAL({precision},{scale})) IS NOT NULL"
                        checks.append(Check(spark_session = self._spark_session,
                            level = CheckLevel.Error,
                            description= "Schema Validation Decimal Type Precision, Scale Check").satisfies(columnCondition = cast_decimal_expr,
                                                                                                            constraintName = f"field[{column_name}] constaint for specific[DECIMAL({precision},{scale})] AND nullable[{nullable}]",
                                                                                                            assertion = assertion_lambda, 
                                                                                                            hint = hint_expr))
                if not nullable:
                    checks.append(Check(spark_session = self._spark_session, level = CheckLevel.Error, description= "Schema Validation Null Check").isComplete(column = column_name))
    ...
    def _fetch_table_schema(self, catalog_type):
        ...
        database = self._schema_config.get(constants.SCHEMA_VALIDATION_DATABASE_KEY, None)
        table = self._schema_config.get(constants.SCHEMA_VALIDATION_TABLE_KEY)
        catalog_name = self._schema_config.get("catalog", None)

        provider = CatalogFactory.get_provider(self._spark_session, catalog_type=catalog_type)
    ...
    def apply_checks(self, df: DataFrame):
        ...
        if self._single_check_mode:
            ...
            checks = Check(spark_session = self._spark_session, level = CheckLevel.Error, description= "Schema Validation")
        else:
            # Each constraint will have its own Check object
            checks = []
        
        catalog_type = self._schema_config.get(constants.SCHEMA_VALIDATION_CATALOG_TYPE_KEY, constants.SCHEMA_VALIDATION_CATALOG_TYPE_SPARK)
        tableSchema = self._fetch_table_schema(catalog_type)
        if catalog_type == constants.SCHEMA_VALIDATION_CATALOG_TYPE_SPARK:
           checks = self._apply_spark_datatype_checks(checks=checks, tableSchema=tableSchema)
        elif catalog_type in [constants.SCHEMA_VALIDATION_CATALOG_TYPE_GLUE,
                              constants.SCHEMA_VALIDATION_CATALOG_TYPE_UNITY,
                              constants.SCHEMA_VALIDATION_CATALOG_TYPE_HIVE]:
           checks = self._apply_catalog_datatype_checks(checks=checks, tableSchema=tableSchema)

        # Apply multi-column UNIQUE constraints
        unique_constraints = self._schema_config.get(constants.SCHEMA_VALIDATION_UNQK_CONSTRAINTS,[])
        if unique_constraints:
            for unique_set in unique_constraints:

                if self._single_check_mode:
                    checks = checks.hasUniqueness(unique_set, lambda x: x==1.0, "Error: Unique Constraint Check failure")
                else:
                    checks.append(Check(spark_session = self._spark_session, level = CheckLevel.Error, description= "Schema Validation Unique Check").hasUniqueness(unique_set, lambda x: x==1.0, "Error: Unique Constraint Check failure"))
        foreign_key_constraints = self._schema_config.get(constants.SCHEMA_VALIDATION_FK_CONSTRAINTS, [])
        if foreign_key_constraints:
                if self._single_check_mode:
                    checks = self.create_foreign_key_checks(checks,df)
                else:
                    checks.append(self.create_foreign_key_checks(Check(spark_session = self._spark_session, level = CheckLevel.Error, description= "Schema Validation Foregin Key Check"),df))
        return checks